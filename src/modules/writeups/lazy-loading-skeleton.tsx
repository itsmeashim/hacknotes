  ...
  return (
    <TableRow className="border-b border-gray-200 dark:border-gray-700">
      <TableCell className="p-4">
        <div className="h-4 w-[500px] bg-gray-200 dark:bg-gray-700 rounded animate-pulse"></div>
      </TableCell>
      <TableCell className="p-4">
...
export const WriteupTableSkeleton: React.FC<{ count?: number }> = ({ count = 20 }) => {
  return (
    <>
...
export const LazyLoadingTrigger: React.FC<{
  ...
  return (
    <div ref={triggerRef} className="h-10 flex items-center justify-center">
      {loading && <div className="h-6 w-6 border-2 border-t-transparent rounded-full animate-spin"></div>}
    </div>
  );
};
      ...
      <table className="w-full">
        ...
        <tbody>
          {writeups.map((writeup) => (
            <tr key={writeup.id} className="border-b border-gray-200 dark:border-gray-700">
              ...
              <td className="p-4">{writeup.publishedAt?.toLocaleDateString()}</td>
