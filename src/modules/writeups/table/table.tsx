...
export function WriteupsTable() {
	...
	const {
		...
		isLoading,
		isFetching,
	} = api.writeups.getWriteups.useQuery(
		{
					...
					<div className="mt-4 flex items-center justify-between gap-3">
						<div className="text-muted-foreground text-sm">
							{writeups && writeups.total > 0 && (
								<span>
									Showing {writeups.items.length} of {writeups.total} writeups
								</span>
							)}
						</div>
						...
						<div className="flex items-center gap-3">
							{writeups && writeups.pageCount > 1 && (
								<div className="flex items-center justify-end space-x-2 rounded-md border border-muted/30 bg-muted/20 p-1">
									<Button
										variant="outline"
										size="sm"
										onClick={() => {
