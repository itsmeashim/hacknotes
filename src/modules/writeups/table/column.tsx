	...
	cell: ({ row }) => {
		return (
			<div className="flex items-center justify-between">
				<Link
					target="_blank"
					className="block w-[500px] text-wrap text-amber-200"
				...
				>
					...
					{row.original.source === "hackerone" && (
						<Tooltip>
							<TooltipTrigger>
								<div
									className="mr-2 inline-flex h-4 w-4 items-center justify-center rounded-full bg-green-500"
									onClick={(e) => {
										e.preventDefault();
				...
				{row.original.severity && row.original.severity !== "none" && (
					<Badge
						variant={
							row.original.severity as "critical" | "high" | "medium" | "low"
						}
